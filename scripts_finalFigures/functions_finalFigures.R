...
makeHeatmapPlot<-function(res, numSamplesSignificant, lfcVal, padjVal, direction,
  ...
  pdf(paste0(workDir, runName,"/custom/finalFigures/upregulatedOnArms/",setName,"hclust_heatmap_sigSamples",
  ...
  ht<-Heatmap(mat_lfc,show_row_names=F, cluster_columns=T, cluster_rows=T, show_row_dend = F,
              ...
              col=circlize::colorRamp(breaks = c(0, 0, 2), colors = c("white", "white", "red")))
  ht<-draw(ht)
  dev.off()
  return(ht)
}
