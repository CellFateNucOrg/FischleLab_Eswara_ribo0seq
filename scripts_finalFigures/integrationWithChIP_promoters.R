...
#gr1<-resize(gr,width=width(gr)+300,fix="end")
gr1<-promoters(gr,upstream=300,downstream=100)
for(i in 1:nrow(beddf)){
  bed<-import(beddf$bedFiles[i])
  seqlevelsStyle(bed)<-"UCSC"
  cname<-paste0(beddf$target[i],"_peaks")
  mcols(gr1)[cname]<-countOverlaps(gr1,bed)
}

gr1
peakCols<-grep("_peaks",colnames(mcols(gr1)))
gr1$peakTypeNumber<-rowSums(data.frame(mcols(gr1)[,peakCols])>0)
...
for(i in 1:nrow(bwdf)){
  ...
  seqlevels(bw)<-seqlevels(gr1)
  cov<-coverage(bw,weight="score")
  cname<-paste0(bwdf$target[i],"_signal")
  print(cname)
  gr1<-binnedAverage(gr1,cov,varname=cname,na.rm=T)
}

# for(i in 1:nrow(bwdf)){
#   print(bwdf$target[i])
#   print(bwdf$totalScore[i])
#   mcols(gr1)[paste0(bwdf$target[i],"_signalNorm")]<-NULL#round(1e9*as.matrix(mcols(gr1)[paste0(bwdf$target[i],"_signal")])/bwdf$totalScore[i],0)
# }

saveRDS(data.frame(gr1),paste0(workDir,runName,"/custom/rds/",prefix,"_ChIPpeaks_us300genebody.results_annotated.RDS"))

res<-readRDS(paste0(workDir,runName,"/custom/rds/",prefix,"_ChIPpeaks_us300genebody.results_annotated.RDS"))
...
p
ggsave(filename = paste0(workDir, runName, "/custom/finalFigures/integrationWithChIP/us300genebody_lfcByGroup_autosomalArmTripleChIPpeaks",
...
p
ggsave(filename = paste0(workDir, runName, "/custom/finalFigures/integrationWithChIP/promoters_lfcByGroup_autosomalArmTripleChIPpeaks",
